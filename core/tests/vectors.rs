...
const VECTORS_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/vectors.json");
...
fn generate_recovery_valid() -> Result<Fixture, String> {
    ...
    let devices = vec![device.manifest_entry(eisen_core::identity::DeviceStatus::Active)];
    let epoch_roots: BTreeMap<u64, eisen_core::epoch::EpochRoot> = BTreeMap::new();
...
fn generate_fixtures() -> Result<Vec<Fixture>, String> {
...
fn merge_actions(actions: &[Value]) -> Result<Option<eisen_core::Task>, String> {
    let mutations = actions_to_mutations(actions)?;
    eisen_core::merge_mutations(&mutations).map_err(|e| e.to_string())
}

fn task_title(task: &Option<eisen_core::Task>) -> Result<String, String> {
    task.as_ref()
...
fn verify_merge_drop(f: &Fixture) -> Result<(), String> {
    ...
    dropped.pop();
    let full_title = task_title(&eisen_core::merge_mutations(&full).map_err(|e| e.to_string())?)?;
    let dropped_title =
        task_title(&eisen_core::merge_mutations(&dropped).map_err(|e| e.to_string())?)?;
    assert_string(
...
fn verify_fixture(f: &Fixture) -> Result<(), String> {
    match f.id.as_str() {
...
#[test]
fn run_vectors() {
    if std::env::var_os("EISEN_GENERATE_VECTORS").is_some() {
        let fixtures = generate_fixtures().unwrap_or_else(|e| panic!("generate: {e}"));
        let file = std::fs::File::create(VECTORS_PATH).unwrap();
        serde_json::to_writer_pretty(file, &fixtures).unwrap();
    } else {
        let data = std::fs::read_to_string(VECTORS_PATH).unwrap();
        let fixtures: Vec<Fixture> = serde_json::from_str(&data).unwrap();
        for f in fixtures {
            verify_fixture(&f).unwrap_or_else(|e| panic!("{}: {}", f.id, e));
        }
    }
}
