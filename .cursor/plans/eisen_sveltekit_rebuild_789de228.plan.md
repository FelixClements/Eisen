---
name: Eisen SvelteKit Rebuild
overview: Research and plan a fresh SvelteKit rebuild with Tailwind v4, Konsta UI v5, Better Auth (email/password), simplified client-side E2EE for tasks, and Web Push notifications — dialing back the Rust core and full cryptographic protocol while keeping the local-first + encrypted-blob sync model.
todos:
  - id: research-doc
    content: Write docs/research/sveltekit-konsta-better-auth-rebuild.md (stack, auth+crypto model, D1 schema, notifications, port map)
    status: completed
  - id: scaffold
    content: Create clients/web/ fresh SvelteKit + Tailwind v4 + Konsta v5 + adapter-cloudflare scaffold
    status: completed
  - id: better-auth
    content: Implement Better Auth email/password on D1 with hooks.server.ts and auth tables migration
    status: completed
  - id: vault-crypto
    content: Port vault/crypto layer with two-layer auth (session + vault passphrase unlock)
    status: completed
  - id: konsta-ui
    content: Build Konsta app shell and Eisenhower task screens (port UX from android-app-ui-ux research)
    status: completed
  - id: encrypted-sync
    content: Port encrypted sync + recovery APIs scoped to Better Auth user.id
    status: completed
  - id: notifications
    content: Implement Web Push wake-clock pipeline with E2EE-preserving service worker handlers
    status: completed
isProject: false
---
